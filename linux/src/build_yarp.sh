...
run_in_chroot build_chroot "echo 'nameserver 8.8.4.4' | tee -a /etc/resolv.conf"
...
run_in_chroot build_chroot "mkdir -p $CHROOT_BUILD && cd $CHROOT_BUILD && $CMAKE $YARP_CMAKE_OPTIONS -DCMAKE_INSTALL_PREFIX=/usr $CHROOT_SRC" || exit 1
...
echo "Fixing yarp.ini in path.d"
# this fixes missing yarp.ini file
if [ ! -e "${YARP_INI_PATH}/${YARP_INI_FILE}" ]
then
  run_in_chroot build_chroot "mkdir -p $YARP_INI_PATH & touch ${YARP_INI_PATH}/${ICUB_INI_FILE}"
  run_in_chroot build_chroot "echo ###### This file is automatically generated by CMake. >> ${YARP_INI_PATH}/${YARP_INI_FILE}"
  run_in_chroot build_chroot "echo [search yarp] >> ${YARP_INI_PATH}/${YARP_INI_FILE}"
  run_in_chroot build_chroot "echo path \"/usr/share/yarp\">> ${YARP_INI_PATH}/${YARP_INI_FILE}"
fi
